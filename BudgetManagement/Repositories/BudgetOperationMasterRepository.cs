...
namespace BudgetManagement.Repositories
{
    ...
    public class BudgetOperationItem
    {
        ...
        public bool IsSendDone { get; set; }
    ...
    public class BudgetOperationMasterRepository
    {
            ...
            IDbConnection conn, string year, string inputScreen, string companyCode, string deptCode, string sectionCode, string targetMonth)
        {
            ...
            // 🌟 選択された「入力画面」に応じて、取得する組織レベルとSQLを切り替える
            if (inputScreen == "360")
            {
                ...
                sql.AppendLine(@"
                    SELECT 
                        ...
                        (SELECT t.staff_name FROM m_tanto t WHERE t.staff_type = '03' AND t.staff_code = u.staff_handler_code) AS StaffHandlerName,
                        COALESCE(u.send_flag, false) AS IsSendFlag, u.send_handler_code AS SendHandlerCode,
                        (SELECT t.staff_name FROM m_tanto t WHERE t.staff_type = '03' AND t.staff_code = u.send_handler_code) AS SendHandlerName,
                        COALESCE(u.sent_flag, false) AS IsSendDone,
                        CASE WHEN u.last_sent_date IS NULL THEN NULL ELSE TO_CHAR(TO_DATE(u.last_sent_date, 'YYYYMMDD'), 'YYYY/MM/DD') || ' ' || COALESCE(TO_CHAR(TO_DATE(u.last_sent_time, 'HH24MISS'), 'HH24:MI:SS'), '') END AS LastSendDateTime,
            ...
            else if (inputScreen == "340")
            {
                ...
                sql.AppendLine(@"
                    SELECT 
                        s.staff_code AS OrgCode, t.staff_name AS OrgName,
                        COALESCE(u.staff_input_complete_flag, false) AS IsStaffInputCompleted, u.staff_handler_code AS StaffHandlerCode,
                        (SELECT t2.staff_name FROM m_tanto t2 WHERE t2.staff_type = '03' AND t2.staff_code = u.staff_handler_code) AS StaffHandlerName,
                        COALESCE(u.send_flag, false) AS IsSendFlag, u.send_handler_code AS SendHandlerCode,
                        (SELECT t2.staff_name FROM m_tanto t2 WHERE t2.staff_type = '03' AND t2.staff_code = u.send_handler_code) AS SendHandlerName,
                        COALESCE(u.sent_flag, false) AS IsSendDone,
                        CASE WHEN u.last_sent_date IS NULL THEN NULL ELSE TO_CHAR(TO_DATE(u.last_sent_date, 'YYYYMMDD'), 'YYYY/MM/DD') || ' ' || COALESCE(TO_CHAR(TO_DATE(u.last_sent_time, 'HH24MISS'), 'HH24:MI:SS'), '') END AS LastSendDateTime,
                        s.company_code AS CompanyCode, '99999999' AS DeptCode, '99999999' AS SectionCode, s.staff_code AS StaffCode, 99 AS TargetMonth
                    FROM ym_soshiki s
                    JOIN m_tanto t ON t.staff_type = '03' AND t.staff_code = s.staff_code
                    LEFT JOIN ym_unyou u ON u.input_screen = @InputScreen AND u.fiscal_year = s.fiscal_year AND u.company_code = s.company_code AND u.staff_code = s.staff_code
            ...
            else if (inputScreen == "330")
            {
                ...
                sql.AppendLine(@"
                    SELECT 
                        ...
                        COALESCE(u.sent_flag, false) AS IsSendDone,
                        CASE WHEN u.last_sent_date IS NULL THEN NULL ELSE TO_CHAR(TO_DATE(u.last_sent_date, 'YYYYMMDD'), 'YYYY/MM/DD') || ' ' || COALESCE(TO_CHAR(TO_DATE(u.last_sent_time, 'HH24MISS'), 'HH24:MI:SS'), '') END AS LastSendDateTime,
                        s.company_code AS CompanyCode, '99999999' AS DeptCode, s.section_code AS SectionCode, '99999999' AS StaffCode, 99 AS TargetMonth
                    FROM ym_soshiki s
                    JOIN m_mei m ON m.name_id = 'SSKIZOKU3' AND m.name_code != '00000000' AND m.name_code = s.section_code
                    LEFT JOIN ym_unyou u ON u.input_screen = @InputScreen AND u.fiscal_year = s.fiscal_year AND u.company_code = s.company_code AND u.section_code = s.section_code
            ...
            else
            {
                ...
                sql.AppendLine(@"
                    SELECT 
                        ...
                        COALESCE(u.sent_flag, false) AS IsSendDone,
                        CASE WHEN u.last_sent_date IS NULL THEN NULL ELSE TO_CHAR(TO_DATE(u.last_sent_date, 'YYYYMMDD'), 'YYYY/MM/DD') || ' ' || COALESCE(TO_CHAR(TO_DATE(u.last_sent_time, 'HH24MISS'), 'HH24:MI:SS'), '') END AS LastSendDateTime,
                        s.company_code AS CompanyCode, s.dept_code AS DeptCode, '99999999' AS SectionCode, '99999999' AS StaffCode, 99 AS TargetMonth
                    FROM ym_soshiki s
                    JOIN m_mei m ON m.name_id = 'SSKIZOKU20' AND m.name_code != '00000000' AND m.name_code = s.dept_code
                    LEFT JOIN ym_unyou u ON u.input_screen = @InputScreen AND u.fiscal_year = s.fiscal_year AND u.company_code = s.company_code AND u.dept_code = s.dept_code
        ...
        {
            // SELECT時に持たせた非表示キー (CompanyCode, DeptCode, SectionCode, StaffCode, TargetMonth) を使って安全にUPSERT
            string sql = @"
